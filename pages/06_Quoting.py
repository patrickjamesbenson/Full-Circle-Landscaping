...
if st.button("Add line item") and item:
    ...
    if item_id == 0:
        ...
        conn.execute("""INSERT INTO quote_items(quote_id, item_id, description, qty, unit_price, line_total) VALUES (?,?,?,?,?,?)""",
...
dfi = pd.read_sql_query("""SELECT qi.id, qi.description, qi.qty, qi.unit_price, qi.line_total FROM quote_items qi WHERE qi.quote_id=?""", conn, params=(qid,))
